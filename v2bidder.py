...
class BidderGUI:
    ...
    # -------- REMOVE THIS FUNCTION FOR FINAL VERSION -------- #
    def build_connection_ui(self):
        Label(self.root, text="Server IP",
    ...
    def build_main_ui(self):
        Label(self.root, text="Your Name",
              bg="#1e1e1e", fg="white").pack()
        self.name_entry = Entry(self.root)
        self.name_entry.pack()

        Label(self.root, text="Bid Amount",
              bg="#1e1e1e", fg="white").pack()
        self.bid_entry = Entry(self.root)
        self.bid_entry.pack()

        self.bid_button = Button(self.root, text="Place Bid",
                                 command=self.place_bid)
        self.bid_button.pack(pady=5)

        self.message_label = Label(self.root,
                                   bg="#1e1e1e",
                                   fg="cyan")
